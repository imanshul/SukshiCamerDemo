...
public class CameraSourcePreview extends ViewGroup {
    ...
    protected void dispatchDraw(Canvas canvas) {
        mClipPath.reset();
        float radius = Math.min((float) getMeasuredWidth() / 2f, (float) getMeasuredHeight() / 2f)-10;
        mClipPath.addCircle((float) getMeasuredWidth() / 2f, (float) getMeasuredHeight() / 3f, radius - 6, Path.Direction.CCW);
        paint.setStrokeWidth(8);
        paint.setStyle(Paint.Style.STROKE);
        canvas.drawCircle((float) getMeasuredWidth() / 2f, (float) getMeasuredHeight() / 3f, radius, paint);
